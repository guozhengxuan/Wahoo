...
class Bench:
    ...
    def _run_single_wahoo(self, hosts, bench_parameters, node_parameters, ts, protocol='wahoo', debug=False):
        ...
        # Check the first 3 non-faulty nodes' log files
        ...
        while elapsed_time < max_wait_time:
            # Check completion status for each monitored host
            # Complete when ANY one of the monitored hosts completes
            for i, host in enumerate(monitor_hosts):
                # Get the first node index for this host
                node_idx = hosts.index(host) * node_instance
                monitor_log = PathMaker.remote_log_file(node_idx, ts)
                if self._check_wahoo_completion(host, monitor_log):
                    Print.info(f'{protocol} benchmark completed on host {host} after {elapsed_time} seconds')
                    # Give a bit more time for all nodes to finish their final outputs
                    sleep(5)
                    break
            else:
                # No host completed yet, continue waiting
                sleep(check_interval)
                elapsed_time += check_interval
                continue
            # If we broke out of the for loop, break out of the while loop
            break
        else:

...
class Settings:
    def __init__(self, key_name, key_path,accesskey_path, consensus_port,instance_type, aws_regions, repo_url, repo_branch, repo_name):
        regions = aws_regions if isinstance(
            aws_regions, list) else [aws_regions]
        inputs_str = [
            key_name, key_path, instance_type, repo_url, repo_branch, repo_name
        ]
        inputs_str += regions
